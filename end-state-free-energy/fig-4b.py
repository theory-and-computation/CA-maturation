...
labels = [f"State {d}" for d in directories_sorted]
n_states = len(labels)
...
for energy_label, col_index in energy_terms.items():
  ...
  # Plot for energy breakdowns

#    plt.savefig(os.path.join(output_dir, f"{out_stem}_heatmap.png"), dpi=700)
    plt.close()
    ...
    if energy_label == "Total":
        ...
        for region_label, (start, end) in regions.items():
            ...
            ytick_positions = [0, 25, 16, 8, 31.5]  
